...
class ResNetIncrementalCIFARExperiment(IncrementalCIFARExperiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        grow_methods = ["none", "pm_min", "kaiming_normal", "zero", "fixed_with_noise", "median_clipped"]
        ...
        self.bn_update_freq = access_dict(exp_params, "bn_update_freq", default=self.topology_update_freq, val_type=int)
        self.bn_drop_factor = access_dict(exp_params, "bn_drop_factor", default=self.drop_factor, val_type=float)
        self.exclude_bn_bias = access_dict(exp_params, "exclude_bn_bias", default=False, val_type=bool)
    ...
    def train(self, train_dataloader: DataLoader, test_dataloader: DataLoader, val_dataloader: DataLoader,
        ...
        for e in range(self.current_epoch, self.num_epochs):
            ...
            for step_number, sample in enumerate(train_dataloader):
                ...
                if self.use_cbpw_bn and (self.current_minibatch % self.bn_update_freq) == 0:
